...
upload is isolated to the tag/manual release workflow, whose write permission
is limited to `contents`. This follows GitHub's reusable-workflow permission
rule that nested workflows may only retain or reduce permissions:

<https://docs.github.com/en/actions/reference/workflows-and-actions/reusing-workflow-configurations>

Old pull-request runs are cancelled, while the serialized packaged soak keeps
one evidence run at a time. This avoids wasting runners on obsolete PR commits
without cancelling a release measurement halfway through. Build outputs and
test reports are uploaded as artifacts rather than placed in dependency caches;
...
| --- | --- | --- | --- |
| Fast | PR and `main` push | whitespace, file-size policy, semantic tests, focused frontend contract tests, Rust tests, production build, local interaction performance | blocks merge |
| Full | release candidate | complete frontend suite, Rust tests, build, strict interaction performance | blocks release packaging |
| Windows package | PR/main after fast | native Windows package and bundle shape verification | blocks Windows target |
...
the Windows package and long soak to remain platform-specific.
...
5. Keep release publishing outside pull-request workflows; use a version tag or
   explicit manual dispatch after the full gate.
