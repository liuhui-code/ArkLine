  ...
  return (
    <div className="app-shell" data-bottom-layout-token={bottomLayoutToken} data-open-document-count={documentsRef.current.documentCount()} data-open-tab-count={openTabs.length}>
      ...
      <SearchWorkspaceOverlayController
        activeOverlay={activeOverlay} workspace={workspace} workspaceApi={workspaceApi} activePath={activePath} workspaceIndex={workspaceIndexRef.current} documentsRef={documentsRef} tabsRef={tabsRef} getActiveContent={getActiveContent} getEditorSelectedText={editorSelectionRuntimeRef.current.getSelectedText} indexVersionKey={`${workspaceIndexState.indexedAt ?? ""}:${workspaceIndexState.status}`} partialNotice={derived.workspacePartialNotice}
