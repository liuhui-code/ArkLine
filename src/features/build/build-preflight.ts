  ...
  if (input.target !== "har") {
    ...
    if (input.project.productSigning.length > 0 && !signing?.signingConfig) {
      issues.push(issue({
        ...
        message: `Product ${input.product} has no signing configuration; the artifact will be unsigned.`,
        hint: "Configure app.signingConfigs when you need a signed artifact.",
      }));
