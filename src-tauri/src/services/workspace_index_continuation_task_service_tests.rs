...
#[test]
fn schedules_deep_refresh_after_background_workspace_refresh() {
    let result = WorkspaceIndexTaskResult {
        root_path: "/workspace".to_string(),
        kind: "refresh-workspace".to_string(),
        status: "partial".to_string(),
        ...
        refresh_result: Some(crate::models::workspace::WorkspaceIndexRefreshResult {
            state: crate::models::workspace::WorkspaceIndexState {
                status: crate::models::workspace::WorkspaceIndexStatus::Partial,
                root_path: Some("/workspace".to_string()),
                file_paths: vec!["A.ets".to_string()],
                symbols: Vec::new(),
                indexed_at: Some(200),
    ...
    assert_eq!(
        tasks[0].reason,
        "full-refresh-deep:background-refresh-after-open"
    );
...
#[test]
fn schedules_catalog_deep_refresh_without_readding_path_arrays() {
    let result = WorkspaceIndexTaskResult {
        root_path: "/workspace".to_string(),
        ...
        started_at: Some(100),
        finished_at: Some(200),
        message: Some(CATALOG_DEEP_REFRESH_MESSAGE.to_string()),
        error: None,
        refresh_result: None,
        refresh_continuation: None,
        sdk_path: None,
