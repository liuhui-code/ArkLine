use crate::indexer_sidecar::{IndexerStubRefreshResult, IndexerTaskKey};
...
impl IndexerHostRuntime {
    ...
    where
        F: FnMut() -> bool,
    {
        self.refresh_stub_chunk_with_priority(
            task,
            indexed_generation,
            changed_paths,
            removed_paths,
    ...
    pub(crate) fn refresh_stub_chunk_with_priority<F>(
        &self,
        task: IndexerTaskKey,
        indexed_generation: u64,
        changed_paths: Vec<String>,
            ...
            Ok(mut result) => {
                let Some(descriptor) = result.publication_artifact.take() else {
                    if !session.supports_writer_actor_publication() {
                        ...
                        return IndexerStubRefreshAttempt::Applied(result);
                    }
                    self.finish_failure(
                        IndexerRequestKind::StubRefresh,
                        "Indexer stub prepare omitted publication artifact".to_string(),
                    );
                    return IndexerStubRefreshAttempt::Unavailable;
                };
                match self.writer.publish(
                    ...
                    {
                        self.finish_superseded(session, IndexerRequestKind::StubRefresh);
                        IndexerStubRefreshAttempt::Cancelled
                    }
                    WorkspaceIndexPublicationAttempt::Failed(error) => {
                        self.finish_failure(IndexerRequestKind::StubRefresh, error);
